...
func (h *Handle) OpenBuffer(data []byte) int {
	...
	code := int(C.libraw_open_buffer(h.ptr, h.cbuf, C.size_t(len(data))))
	if code != 0 {
		h.freeBuffer()
	}
	return code
...
//
// libraw_recycle zeroes all C-side state including callback pointers, so any
// previously registered Go handlers are re-installed afterwards.
...
func convertProcessedImage(img *C.libraw_processed_image_t, errc C.int) (ProcessedImage, int) {
	...
	if img.data_size > 0 {
		// data_size is an unsigned int. C.GoBytes takes a C.int length, which is
		// 32-bit and would wrap negative for buffers above 2 GiB; copy via a Go
		// int length instead. int is 64-bit on the supported platforms and holds
		// the full uint32 range. The guard reports a value that cannot be
		// represented (only reachable on a 32-bit build) rather than panicking in
		// unsafe.Slice with a negative length.
		const maxInt = int(^uint(0) >> 1)
		if uint64(img.data_size) > uint64(maxInt) {
			return ProcessedImage{}, int(C.LIBRAW_UNSUFFICIENT_MEMORY)
		}
		src := unsafe.Slice((*byte)(unsafe.Pointer(&img.data[0])), int(img.data_size))
		pi.Data = append([]byte(nil), src...)
	}
	return pi, int(errc)
}
